...
DELETE FROM OrderDetail WHERE orderID = "OD000005";
DELETE FROM OrderDetail WHERE orderID = "OD000006";
DELETE FROM OrderDetail WHERE orderID = "OD000007";
DELETE FROM OrderDetail WHERE orderID = "OD000008";
DELETE FROM OrderDetail WHERE orderID = "OD000009";
DELETE FROM OrderDetail WHERE orderID = "OD000010";
...
DELETE FROM ServiceDetail WHERE orderID = "OD000008";
DELETE FROM ServiceDetail WHERE orderID = "OD000009";
...
-- Java: void registerEmployee(Employee emp)
INSERT INTO Account(accountID, username, password, role) VALUES (?, ?, ?, ?);
-- Java: void insertEmployee(Employee emp)
insert into Employee(empID, empName, empGender, empDOB, empAdress, empPhone, empEmail) values (?,?,?,?,?,?,?);
-- Java: String generateNewID()
SELECT accountID FROM account WHERE accountID LIKE 'EMP%' ORDER BY accountID DESC LIMIT 1;
-- Java: Employee getEmployeeByID(String id)
SELECT * FROM Account JOIN Employee ON accountID = empID WHERE empID = ?;
-- Java: boolean checkUserNameExist(String username)
select * from Account  where username =?;
-- Java: ArrayList<Employee> getAllEmployee()
SELECT * FROM Account JOIN Employee ON Account.accountID = Employee.empID WHERE Account.role=1;
-- Java: ArrayList<Employee> getAllManager()
SELECT * FROM Account JOIN Employee ON Account.accountID = Employee.empID WHERE Account.role=3;
-- Java: ArrayList<Customer> getAllCustomer()
SELECT * FROM Account JOIN Customer ON Account.accountID = Customer.cusID;   

-- Class: CustomerRepository 
-- Java: Customer login(String username, String password)
SELECT * FROM Account JOIN Customer ON Account.accountID = Customer.cusID WHERE Account.username = "user1" AND Account.password = "123";
-- Java: void registerCustomer(Customer a)
INSERT INTO Account(accountID, username, password, role) VALUES ('CUS000015', 'test', '123', 0);
-- Java: Customer getCustomerByID(String id) 
SELECT * FROM Account JOIN Customer ON accountID = cusID WHERE cusID = "CUS000015";
-- Java: String generateNewID()
...
-- Java: void insertCustomer(Customer a)
insert into Customer(cusID, cusName, cusDOB, cusPhone, cusEmail, cusCCCD, cusType) 
values ('CUS000015','Tester','2001-08-15','0363012456','congthanh15082001@gmail.com','52204004285',1);

-- Class: EmployeeRepository
-- Java: Employee login(String username, String password)
SELECT * FROM Account JOIN Employee ON Account.accountID = Employee.empID WHERE Account.username = ? AND Account.password = ?;

-- Class: FoodCartRepository 
-- Java: void insertFoodOrder(String orderID, String txt)
Insert into OrderDetail (orderID, foodID, orTime, Amount, orStatus) Value(?,?,?,?,?);
-- Java: List<FoodCart> getListCart(String txt)
Select foodID, foodName, foodPrice From Food Where foodID = ?;

-- Class: FoodRepository 
-- Java: List<Food> getListFood()
Select * from Food;

-- Class: ManagerRepository 
-- Java: List<ContractOrder> getAllPendingContract()
SELECT cd.orderID, cd.timeRegister, c.cusID, c.cusName, cd.roomID, cd.timeIn, cd.timOut, cd.people
FROM Customer c INNER JOIN `Order` o ON c.cusID = o.cusID INNER JOIN ContractDetail cd ON o.orderID = cd.orderID 
WHERE cd.status = 0;
-- Java: List<ContractOrder> getAllAcceptedContract()
SELECT cd.orderID, cd.timeRegister, c.cusID, c.cusName, cd.roomID, cd.timeIn, cd.timOut, cd.people
FROM Customer c INNER JOIN `Order` o ON c.cusID = o.cusID INNER JOIN ContractDetail cd ON o.orderID = cd.orderID
WHERE cd.status = 1 and o.orStatus = 0;
-- Java: List<ContractOrder> getAllPaidContract()
SELECT cd.orderID, cd.timeRegister, c.cusID, c.cusName, cd.roomID, cd.timeIn, cd.timOut, cd.people
FROM Customer c INNER JOIN `Order` o ON c.cusID = o.cusID INNER JOIN ContractDetail cd ON o.orderID = cd.orderID
WHERE o.orStatus = 1;
-- Java: void acceptOrder(String orderID, String cusID, String roomID)
UPDATE ContractDetail SET status = 1
WHERE orderID = ? AND orderID IN (SELECT orderID FROM `Order` WHERE cusID = ?) AND roomID = ?;
-- Java: void updateRoomStatus(String orderID, String roomID)
UPDATE Room JOIN ContractDetail ON Room.roomID = ContractDetail.roomID SET Room.roomStatus = CASE
WHEN CURDATE() BETWEEN ContractDetail.timeIn AND ContractDetail.timOut 
THEN 1 ELSE 0 END WHERE ContractDetail.orderID = ? AND ContractDetail.roomID = ?;
-- Java: void removeOrder(String orderID)
delete from ContractDetail `Order` where orderID=?;
delete from `Order` where orderID=?;
-- Java: void Payment(String orderID)
UPDATE `Order` SET orStatus = 1 WHERE orderID = ?;

-- Class: OrderRepository
-- Java: String getOrderID()
SELECT MAX(SUBSTRING(orderID, 3)) AS maxID FROM `Order`;
-- Java: List<Order> getOrderByCustomerID(String cusID)
select * from `Order` where cusID = "CUS000015";
-- Java: String makeOrder(Room room, Contract contract, Customer cus)
INSERT INTO `Order`(orderID, cusID, orDate, orStatus) VALUES (?, ?, ?, ?);
-- Java: void createContractDetail(Room room, Contract contract, String orderID)
Insert INTO ContractDetail(roomID, orderID, timeIn, timOut, people, timeRegister, status) VALUES (?, ? , ?, ?, ?, ?, ?);
-- Java: void setStatusRoom(Room room)
UPDATE Room SET roomStatus = 1 WHERE id = ?;

-- Class: RoomRepository 
-- Java: Room getRoomByID(String id)
select * from Room WHERE roomID = ?;
-- Java: ArrayList<Room> getAllRoom()
select * from Room;
-- Java: ArrayList<Room> getChoosenRoom(String roomInputType, int maxInputPeople)
SELECT * FROM Room WHERE roomStatus = 0 AND roomType = ? AND maxPeople >= ?;

-- Class: ServiceRepository
-- Java: List<Service> getListService()
Select * from Service;
-- Java: List<ServiceCart>  getListServiceCart(String txt)
Select * from Service where serviceID = ?;
-- Java: void insertintoServiceDetail(String txt, String orderID)
Insert into ServiceDetail(orderID, serviceID, serviceTime, Amount, serviceStatus) Values(?,?,?,?,?);

-- Class: YourCartRepository
-- Java: List<FoodCart> getYourCartFood(String orderID)
Select b.orderID, a.foodID, a.foodName, a.foodPrice, b.orTime, b.Amount, b.orStatus 
from Food a join OrderDetail b on a.foodID = b.foodID
where b.orderID = ?;
-- Java: List<ServiceCart> getYourCartService(String orderID)
Select b.orderID, a.serviceID, a.serviceName, a.servicePrice, b.serviceTime, b.Amount, b.serviceStatus 
from Service a join ServiceDetail b on a.serviceID = b.serviceID
where b.orderID = ?;
-- Java: List<RoomCart> getYourCartRoom(String orderID)
Select b.orderID, a.roomID, a.roomName, a.roomType, b.timeRegister, b.timeIn, b.timOut, b.people, a.roomPrice 
from Room a join ContractDetail b on a.roomID = b.roomID
where b.orderID = ?;
-- Java: 
...
Select a.orderID, a.orStatus, b.roomID, b.timeIn ,c.roomName from `Order` a join ContractDetail b join Room c
on a.orderID = b.orderID and b.roomID = c.roomID
Where a.cusID = "CUS000013" and a.orStatus = "0"; 

Select * from `Order` a join ContractDetail b join Room c
on a.orderID = b.orderID and b.roomID = c.roomID
Where a.cusID = "CUS000013" and a.orStatus = "0";
...
Values ('R000014', 'OD000015', '2023-06-24', '2023-06-26', 2, '2023-06-13 11:44:30', 0);

Select * from Room a join ContractDetail b on a.roomID = b.roomID where orderID = "OD000005";
Select * from OrderDetail where orderID = "OD000005";
Select * from ServiceDetail where orderID = "OD000005";